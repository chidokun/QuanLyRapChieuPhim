...
public class PhimModel {
	public static ArrayList<Phim> traCuu(String TenPhim, Date TuNgay, Date DenNgay) throws SQLException {
		...
		while (rs.next()) {
			arr.add(new Phim(
					rs.getString("MaPhim"), rs.getString("TenPhim"), rs.getInt("ThoiLuong"), 
					...
					rs.getInt("GioiHanTuoi"),rs.getDouble("DoanhThu"), rs.getString("CacDienVien"),rs.getInt("TrangThai")));
		}
	...
	public static Phim layThongTin(String maPhim) throws SQLException {
		...
		Phim p = new Phim(maPhim,rs.getString(1), rs.getInt(2), rs.getString(3), rs.getInt(4), rs.getString(5),
				rs.getDate(6), rs.getString(7), rs.getString(8), rs.getInt(9), rs.getDouble(10),rs.getString(11), rs.getInt(12));
	...
	public static boolean them(Phim p,ArrayList<SuatPhim> aSuatPhim) throws SQLException {
		...
		try {
			...
			Database.callStoredUpdate("sp_ThemPhim",p.getMaPhim(),p.getTenPhim(),p.getThoiLuong(),p.getMaNhaSX(),p.getNamSX(),
										p.getMaNgonNgu(),p.getNgayKhoiChieu(),p.getTomTat(),p.getMaNuocSX(),p.getGioiHanTuoi(),p.getCacDienVien());
	...
	public static boolean sua(Phim p) throws SQLException {
		...
		try {
			con.setAutoCommit(false);
			Database.callStoredUpdate("sp_SuaPhim",p.getMaPhim(),p.getTenPhim(),p.getThoiLuong(),p.getMaNhaSX(),p.getNamSX(),
										p.getMaNgonNgu(),p.getNgayKhoiChieu(),p.getTomTat(),p.getMaNuocSX(),p.getGioiHanTuoi(),p.getCacDienVien(),p.getTrangThai());
